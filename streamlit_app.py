import streamlit as st
import pandas as pd
import requests
from datetime import datetime
import time

# 1. 網頁基礎設定
st.set_page_config(page_title="👫 我們的甜蜜帳本", layout="centered", page_icon="💰")

# 2. 從 Secrets 讀取網址
try:
    API_URL = st.secrets["API_URL"]
    SHEET_URL = st.secrets["SHEET_URL"]
except Exception:
    st.error("❌ 偵測不到 Secrets 設定，請確認 Streamlit 後台已填入 API_URL 與 SHEET_URL")
    st.stop()

# 3. 讀取資料函數 (加強版：徹底解決不更新問題)
@st.cache_data(ttl=2)  # 緩存僅保留 2 秒
def load_data(url):
    try:
        # 將網址轉為 CSV 匯出格式
        csv_url = url.replace('/edit', '/export?format=csv')
        # 【關鍵】加入時間戳記參數，強迫 Google 伺服器交出最新檔案，不要給舊快取
        csv_url += f"&t={int(time.time())}"
        
        data = pd.read_csv(csv_url)
        # 確保必要的欄位都存在
        for col in ["日期", "類型", "金額", "項目", "分類", "餘額"]:
            if col not in data.columns:
                data[col] = None
        return data
    except Exception as e:
        return pd.DataFrame(columns=["日期", "類型", "金額", "項目", "分類", "餘額"])

# 執行讀取
df = load_data(SHEET_URL)

# 4. 數據統計計算
balance = 0
monthly_exp = 0
if not df.empty:
    df['金額'] = pd.to_numeric(df['金額'], errors='coerce').fillna(0)
    df['餘額'] = pd.to_numeric(df['餘額'], errors='coerce').fillna(0)
    # 取最後一行的餘額
    balance = df["餘額"].iloc[-1]
    # 計算本月支出
    this_month = datetime.now().strftime("%Y-%m")
    df['日期'] = df['日期'].astype(str)
    monthly_df = df[(df['日期'].str.startswith(this_month)) & (df['類型'] == '支出')]
    monthly_exp = monthly_df['金額'].sum()

# 5. 介面顯示
st.title("👫 我們的甜蜜帳本 v5.1")

col1, col2 = st.columns(2)
col1.metric("目前總餘額", f"${balance:,.0f}")
col2.metric("本月總消費", f"${monthly_exp:,.0f}")

st.divider()

# 6. 輸入表單
with st.form("input_form", clear_on_submit=True):
    date = st.date_input("選擇日期", datetime.now())
    r_type = st.radio("交易類型", ["支出", "存款"], horizontal=True)
    
    if r_type == "支出":
        cat = st.selectbox("分類", ["餐飲美食", "交通運輸", "居家生活", "休閒娛樂", "購物開銷", "醫療保健", "其他"])
    else:
        cat = "(存款)"
        
    item = st.text_input("項目描述 (如：晚餐、領薪水)")
    amount = st.number_input("金額", min_value=0, step=1)
    
    submit = st.form_submit_button("✅ 儲存紀錄並同步")

if submit:
    if item and amount > 0:
        with st.spinner('正在同步至雲端試算表...'):
            new_balance = balance + amount if r_type == "存款" else balance - amount
            payload = {
                "日期": date.strftime("%Y-%m-%d"),
                "類型": r_type, 
                "金額": float(amount), 
                "項目": item, 
                "分類": cat, 
                "餘額": float(new_balance)
            }
            
            try:
                # 發送到 Google Apps Script
                response = requests.post(API_URL, json=payload, timeout=10)
                if "Success" in response.text:
                    st.success("✅ 已成功存入！網頁即將重新整理...")
                    # 【關鍵】存檔成功後立刻清除本地緩存
                    st.cache_data.clear()
                    # 延遲一下下確保 Google 那邊已經寫入完成
                    time.sleep(1)
                    st.rerun()
                else:
                    st.error(f"寫入失敗：{response.text}")
            except Exception as e:
                st.error(f"連線錯誤：{e}")
    else:
        st.error("⚠️ 請填寫完整的項目描述與金額！")

# 7. 顯示歷史紀錄
st.subheader("📋 最近紀錄 (最新在最上面)")
if not df.empty:
    # 倒序顯示最新 10 筆
    st.dataframe(df.tail(10).iloc[::-1], use_container_width=True, hide_index=True)
else:
    st.info("目前尚無資料。")

# 8. 簡單圖表
if st.checkbox("📊 顯示本月消費分析"):
    if not df.empty and monthly_exp > 0:
        cat_data = monthly_df.groupby('分類')['金額'].sum()
        st.pie_chart(cat_data)
